...
const controller = class ProductsController {
    ...
    ////////////////////////////////////////////////////////////
    deldateProduct(product, id) {
        return new Promise((resolve,reject) => {
            this.con.query('DELETE FROM products WHERE id = ?', [product, id] , function (err, result) {
                if(err) reject(err);
                resolve();
            });
        });
    ...
    delSizes(product, id) {
        return new Promise((resolve,reject) => {
            this.con.query('DELETE FROM `sizes` SET ? WHERE `product_id` = ? AND `size` = ?', [product, id] , function (err, result) {
                if(err) reject(err);
                resolve();
            });
        });
