...
// delete products
router.get("/products/del", authenticateEmployee(), async (req, res) => {
    const ProductsController = require('../controllers/products.js');
    const Products = new ProductsController();
    let deldateProduct,delSizes;

    try {
        deldateProduct,delSizes = await Products.getProduct(req.query.id);
    } catch (e) {
        deldateProduct,delSizes = false;
    }

    req.session.multer = 0;

    res.render(`${config.views}/dashboard/delProduct.pug`, {deldateProduct,delSizes: deldateProduct,delSizes});
});
